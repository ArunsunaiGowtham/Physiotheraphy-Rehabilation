...
async function testFA() {
  ...
  const iconInfo = await sendCmd('Runtime.evaluate', {
    returnByValue: true,
    expression: `(() => {
      const icons = Array.from(document.querySelectorAll('.site-footer i')).map(i => {
  ...
  console.log('Icon computed info:', iconInfo.result.value);
